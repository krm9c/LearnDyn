...
class data_return():
    ...
    def __init__(self, Config):
        ...
        if self.dataset_id == 'wind':
            import pickle
            with open('/gpfs/jlse-fs0/users/kraghavan/Continual/Wind.p', 'rb') as fp:
                self.dataset = pickle.load(fp)
...
##############################################
    def append_to_experience(self, task_id):
        ...
        if task_id  > 0:
            ...
            # print(self.exp_y_train, self.exp_y_test)
        else:
    ...
    def retreive_data(self, task_id, phase):
        if phase == 'training':
            if task_id > 0:
                ...
                # print("The shapes I am returning are", self.X_train.shape)
